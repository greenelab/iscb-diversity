...
iscb_pubmed %>% filter(year == '2020-01-01',
...
By journals:
```{r}
plot_pubmed_gender <- vector('list', length = n_jours)
i <- 0
for (jour in my_jours){
  i <- i + 1
  plot_pubmed_gender[[i]] <- iscb_pubmed %>%
...
do.call(cowplot::plot_grid, c(plot_pubmed_gender, nrow = 3))
```
...
iscb_pubmed %>% 
  ...
  theme_bw() +
  labs(x = NULL, y = 'Probabilities of being male') +
...
ggplot(iscb_pubmed %>% filter(gender == 'probability_female'),
       aes(x = year)) +
  ...
  labs(x = NULL, y = 'Female Probability')
