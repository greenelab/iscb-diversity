...
The metadata for most Bioinformatics and BMC Bioinformatics papers before 2002 only have initials of first and/or middle names. 
Therefore, we miss many predictions for genders before this year.

In 2000, only two gender predictions were made for two corresponding authors of one paper with probability of each author being male = 1 and 0.99.
...
```{r}
save(iscb_pubmed, pubmed_gender, iscb_gender, file = 'Rdata/iscb-pubmed_gender.Rdata')
```
...
```
Taking all the years together, the Welch two-sample one-sided t-test does not reveal any difference in the mean probability of ISCB speakers predicted to be female compared to that of authors (p = `r round(t_res$p.value, 3)`).
...
ggsave('figs/fig_s1.svg', fig_s1, width = 6, height = 3.5)
```
