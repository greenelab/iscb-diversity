...
nationalize_df <- read_tsv('https://raw.githubusercontent.com/greenelab/wiki-nationality-estimate/0c85554d00aea7b0f75146b32fea4509d0077035/data/NamePrism_results_authors.tsv') %>%
  rename('full_name' = X1) %>%
...
region_levels <- paste(c('Celtic English', 'European', 'East Asian', 'Hispanic', 'South Asian', 'Arabic', 'Hebrew', 'African', 'Nordic', 'Greek'), 'names')
	# European	Finland	Greek	Hispanic	Jewish	Muslim	Nordic	SouthAsian
...
fig_4b <- iscb_pubmed_oth %>%
  ...
  facet_wrap(vars(fct_relevel(region, c('A', 'B', 'C', 'O'))), nrow = 1)
...
fig_s5 <- iscb_pubmed %>%
  ...
  facet_wrap(vars(fct_relevel(region, region_levels_let)))
fig_s5
