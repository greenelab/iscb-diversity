...
Pubmed authors: 7891 last_names out of 29615 were not matched.
...
for (jour in my_jours){
  ...
  pubmed_race[[i]] <- iscb_pubmed %>%
    filter(type == 'Pubmed authors' & journal == jour & (Race != 'All others')) %>%
...
for (conf in my_confs){
  ...
  iscb_race[[i]] <- iscb_pubmed %>%
    filter(type != 'Pubmed authors' & journal == conf & (Race != 'All others')) %>%
    group_by(year, Race, journal) %>%
    summarise(mean_prob = mean(probabilities, na.rm = T)) %>%
    ungroup()
}
...
A simpler figure, supplementary Fig S2?
```{r fig.width=10, fig.height=4}
ggplot(iscb_pubmed %>% filter(Race == 'White' | Race == 'Asian' | Race == 'All others'),
       aes(x = year, color = type, fill = type)) +
  facet_grid(cols = vars(fct_rev(Race))) +
...
fig_2 <- cowplot::plot_grid(fig_2a, fig_2b, fig_2c, fig_2d, labels = 'AUTO')

fig_2

ggsave('figs/racial_makeup.png', fig_2, width = 7, height = 5)
```
