...
pubmed_aff_pmids <- read_tsv('data/names/corresponding-authors.tsv.xz',
                             col_types = cols(fore_name_simple = col_character())) %>%
...
articles %>% 
  ...
  ggplot(aes(x = publication_date, fill = fct_infreq(journal))) +
...
t_w <- t.test(w_iscb, w_pub, alternative = 'greater') %>% 
  broom::tidy() 
...
t_w %>% 
  ...
  print()
```
The Welch two-sample one-sided t-test shows that the mean probability of ISCB speakers predicted to be white is higher than that of authors: t\_`r round(t_w$parameter)`) = `r round(t_w$statistic, 1)`  (p = `r t_w$p.value`).
...
t_a <- t.test(w_iscb, w_pub, alternative = 'less') %>% 
  broom::tidy() 
...
t_a %>% 
  ...
  print()
```
The Welch two-sample one-sided t-test shows that the mean probability of ISCB speakers predicted to be Asian is lower than that of authors: t\_`r round(t_a$parameter)`) = `r round(t_a$statistic, 1)` (p = `r t_a$p.value`).
...
t_o %>% 
  ...
  print()
```
A Welch two-sample t-test also does not reveal any statistically significant difference between the mean probability of ISCB speakers predicted to be in Other categories compared to authors: t\_`r round(t_o$parameter)`) = `r round(t_o$statistic, 1)`  (p = `r t_o$p.value`).


```{r}
w_df %>% 
  select(type, year, probabilities) %>% 
