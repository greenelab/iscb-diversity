...
inte_list <- lapply(large_regions, main_lm)
...
A name coming from the group of honorees has significantly higher probability of being Celtic/English, $\beta_\textrm{Celtic/English} =$ `r get_p(1, 'estimate')` (_P_ = `r get_p(1, 'p.value')`), higher probability of being European, $\beta_\textrm{European} =$ `r get_p(3, 'estimate')` (_P_ = `r get_p(3, 'p.value')`), and lower probability of being East Asian, $\beta_\textrm{East Asian} =$ `r get_p(2, 'estimate')` (_P_ = `r get_p(2, 'p.value')`). The two groups of scientists did not have a significant association with names predicted to be in Other categories (_P_ = `r get_p(4, 'p.value')`).
