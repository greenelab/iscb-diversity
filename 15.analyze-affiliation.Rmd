...
```{r, fig.width = 7*1.5, fig.height = 2.5*1.5}
enrich_df <- world %>% 
...
filtered_obs_exp <- obs_vs_exp_all %>% 
  ...
  mutate(
    ...
    country_name =  as.factor(country_name) %>% fct_reorder(odds_ratio)) %>% 
  arrange(desc(odds_ratio))

plot_obs_exp <- filtered_obs_exp %>%
  mutate(nodds_ratio = - log2(odds_ratio),
         nlower_bound = - log2(lower_bound), 
         nupper_bound = - log2(upper_bound)) %>% 
...
odds_plot_left <- plot_obs_exp_left %>%
  ...
  theme(plot.margin = margin(5.5, 2, 5.5, 5.5, unit = 'pt'))
...
odds_plot_right <- plot_obs_exp_right %>%
  ...
  scale_y_sqrt(breaks = c(0, 1, 4, 16, 50, 100, 200)) +
  scale_color_brewer(type = 'qual', palette = 'Set1') +
  coord_flip() 
