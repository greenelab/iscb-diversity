...
pubmed_aff_df <- pubmed_aff_pmids %>%
  ...
  mutate(OtherCategories = `South Asia` + `Latin America & Caribbean` + `Middle East & North Africa` + `Sub-Saharan Africa`) %>%
  select(-c(countries, country_name, num_affls)) %>% 
  pivot_longer(c(region_levels, OtherCategories),
               names_to = 'region',
...
iscb_aff_df <- read_tsv('data/iscb/keynotes.tsv') %>%
  ...
  mutate(OtherCategories = `Latin America & Caribbean` + `Middle East & North Africa`) %>%
  pivot_longer(c(`North America`:`East Asia & Pacific`, OtherCategories),
               names_to = 'region',
               values_to = 'probabilities')
...
```{r}
table(pubmed_aff_df$`North America`)
```
...
big_cat <- c('North America', 'Europe & Central Asia', 'East Asia & Pacific', 'OtherCategories')
...
fig_5
ggsave('figs/geo_affliation_breakdown.png', fig_5, width = 6.5, height = 5.5)

```

### Supplementary Figure S4 {#sup_fig_s5}
