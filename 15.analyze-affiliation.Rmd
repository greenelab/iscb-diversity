...
pubmed_aff_df <- pubmed_aff_pmids %>% 
  select(pmid, journal, publication_date, year, countries) %>% 
...
```{r}
# table(pubmed_aff_pmids$corresponding)
# table(pubmed_aff_pmids$use_last)
pubmed_aff_df <- pubmed_aff_pmids %>%
  select(pmid, journal, publication_date, year, countries) %>%
  separate_rows(countries) %>%
  add_count(pmid) %>%
  mutate(probabilities = 1 / n) %>%
  left_join(nat_to_reg, by = 'countries') %>%
...
```{r}
# pubmed_aff_df_wide <- pubmed_aff_df %>% 

table(pubmed_aff_df$region)
...
#   })
# 
# top_names %>% write_tsv('data/names/example_affionalities.tsv')
```
...
```{r}
# pubmed_aff_df %>% filter(is.na(African)) %>%
#   count(year, journal) %>%
...
for (jour in my_jours) {
  i <- i + 1
  pubmed_aff[[i]] <- pubmed_aff_df %>%
    pivot_longer(c(`North America`:`Middle East & North Africa`),
               names_to = 'region',
...
check = pubmed_aff[[1]]
bind_rows(pubmed_aff) %>%
  # filter(year < '2020-01-01') %>%
  region_breakdown(category = 'main', journal)
...
iscb_aff_df %>% filter(year == '2011-01-01' & conference == 'PSB')
iscb_aff_df %>% filter(year == '2020-01-01' & conference == 'PSB')
```
...
fig_s4_1 <- bind_rows(pubmed_aff) %>%
  region_breakdown(category = 'sub', journal) +
  theme(legend.position = 'right',
        legend.key.height = unit(3.5, 'mm'),
        legend.margin = margin(0, 1, 0, 1.1, unit='cm')
        )

fig_s4_2 <- bind_rows(iscb_aff) %>%
  filter(year < '2020-01-01') %>%
  region_breakdown(category = 'sub', journal)
